...
// DELETE handler - removes a product from the database.  Added as a 
// convenience to enable DB cleanup.

exports.productDelete = function(request, response) {
    ...
    Product.findOne( {_id: request.body.productId}, 
        ...
        if (err) {
            ...
            // Remove a single occurrence of the product.
            Product.deleteOne( {_id: request.body.productId}, 
                function(err, products) {
                    ...
                    if (err) {
                        ...
                        response.status(200).send("Product removed from the database!");
                    }
                });
        }
